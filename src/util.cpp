...
const char * const BITCOIN_CONF_FILENAME = "kalamazoocoin.conf";
const char * const BITCOIN_PID_FILENAME = "kalamazoocoin.pid";
...
#else
    const char* pszModule = "kalamazoocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kalamazoocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Kalamazoocoin";
#else
    // Unix
    return pathRet / ".kalamazoocoin";
#endif
#endif
}
